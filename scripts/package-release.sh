...
Changes:
- Fnをもう一度押したときは、文字起こしして入力欄へ貼り付けるだけにしました。
- Enterで止めたときは、従来どおり貼り付け後にEnter送信まで行います。
- 無音や短いキャンセルでは、ポップアップをすぐ消してAPI文字起こしをスキップします。
- YouTube一時停止処理を録音開始後の非同期処理にし、Fnの開始/停止操作を待たせにくくしました。
